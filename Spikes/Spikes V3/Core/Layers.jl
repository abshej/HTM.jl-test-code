...
function depolRest!(lyr::LYR)
    for i in eachindex(lyr.mncs)
        depolRest!(lyr.mncs[i])
    end
end

function gds!(lyr::LYR, activeNeurons::Vector{Tuple})
    for i in eachindex(lyr.mncs)
        gds!(lyr.mncs[i], activeNeurons)
    end
end
...
function layer(noOfMNCs::Int, noOfNRNsPerMNC::Int)
    LYR([minicol(noOfNRNsPerMNC) for i in 1:noOfMNCs])
end
